...
function toISO(dateStr) {
  ...
  try {
    ...
    // "Monday March 30th" / "Wednesday April 1st" (Industry TX format — no year)
    const m4 = s.match(/(?:[A-Za-z]+\s+)?([A-Za-z]+)\s+(\d{1,2})(?:st|nd|rd|th)?$/i);
    if (m4) {
      const mo = MONTH_MAP[m4[1].toLowerCase()];
